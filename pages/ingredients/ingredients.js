...
Page({
  data: { t: {
    ...
    ok: '\u63d0\u4ea4\u6210\u529f',
    fail: '\u63d0\u4ea4\u5931\u8d25'
  }, categories: ingredientCategories, activeCategory: 'meat', search: '', ingredients: [], visibleIngredients: [], quantities: {}, selectedCount: 0, selectedTotal: 0, submitting: false },
  ...
  refresh() {
    const quantities = app.getIngredientQuantities()
    const selectedCount = Object.keys(quantities).length
    const selectedTotal = Object.keys(quantities).reduce((sum, id) => sum + Number(quantities[id] || 0), 0)
    this.setData({ ingredients: app.globalData.ingredients, quantities, selectedCount, selectedTotal }, () => this.computeVisible())
  },
  computeVisible() {
    const keyword = (this.data.search || '').trim()
    const quantities = this.data.quantities || {}
    const visibleIngredients = this.data.ingredients
      .filter(item => item.categoryId === this.data.activeCategory && (!keyword || item.name.indexOf(keyword) >= 0))
      .map(item => ({ ...item, quantity: quantities[item.id] || 0, selected: Boolean(quantities[item.id]) }))
    this.setData({ visibleIngredients })
  },
  chooseCategory(event) { this.setData({ activeCategory: event.currentTarget.dataset.id }, () => this.computeVisible()) },
  onSearch(event) { this.setData({ search: event.detail.value }, () => this.computeVisible()) },
  async add(event) {
    const id = event.currentTarget.dataset.id
    const quantities = { ...(this.data.quantities || {}) }
    quantities[id] = Math.min(99, Number(quantities[id] || 0) + 1)
    await app.updateIngredientSelection(quantities)
    this.refresh()
  },
  async minus(event) {
    const id = event.currentTarget.dataset.id
    const quantities = { ...(this.data.quantities || {}) }
    const next = Number(quantities[id] || 0) - 1
